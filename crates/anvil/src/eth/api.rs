...
use alloy_consensus::{
    transaction::eip4844::TxEip4844Variant, Signed, TxEip1559, TxEip2930, TxEnvelope, TxLegacy,
};
...
impl EthApi {
    ...
    async fn validate_tx(&self, tx: &Bytes) -> Result<PoolTransaction> {
        ...
        let transaction = if data[0] > 0x7f {
            // legacy transaction
            match TypedTransaction::decode(&mut data) {
                Ok(transaction) => transaction
                    // .as_legacy()
                    // .ok_or(BlockchainError::FailedToDecodeTransaction)?
                    .clone(),
        ...
        let pool_transaction = PoolTransaction {
            requires,
            provides: vec![to_marker(nonce.into(), *pending_transaction.sender())],
            pending_transaction,
            priority,
        };
    ...
    pub async fn suavex_build_eth_block(
        ...
        Ok(block.execution_envelope(
            ...
            U256::from(block.receipts[0].cumulative_gas_used()),
        ))
    }
